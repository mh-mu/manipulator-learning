...
class Dataset:
    ...
    def append_traj_data_lists(self, data_list, img_list, final_obs_included=False,
                               new_indices=None):
        ...
        assert len(data_list) == len(img_list)
    ...
    def load_to_ram_worker(self, index, normalize_img=False):
        """ load a single index """
    ...
    def load_to_ram_multiind_worker(self, index_range, normalize_img=False):
        """ load multiple indices """
        imgs = []
        for i in index_range:
            img = self.load_to_ram_worker(i, normalize_img)
            imgs.append([img])
        return imgs

    def load_to_ram(self, num_workers=4, normalize_img=False,
        ...
        if self.ram_data is None or load_unloaded:
            ...
            if len(indices_to_load) < 5000:
                data = self.load_to_ram_multiind_worker(indices_to_load, normalize_img)
            else:
            ...
            print('RAM used by dataset: %.1f MB' % ((self.ram_data['img'].nbytes +
    ...
    def new_dataset_from_indices(self, new_dir, indices):
        ...
        for i_s, s in enumerate(with_last):
            ...
            new_dataset.append_traj_data_lists(data_list, img_list, final_obs_included=True)
            print('Copying traj %d of %d to new dataset at %s' % (i_s + 1, len(with_last), new_dir))
        return new_dataset

    def get_data(self, inds, normalize_img=True):
        """ Get some data given some indices """
        if self.ram_data is None:
            imgs = self.load_to_ram_multiind_worker(inds, normalize_img)
            return (
                np.array([item[0] for item in imgs]),
